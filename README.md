...
The first OpenClaw plugin scaffold now wraps this exact safe surface. It stays agent-safe by exposing only `/vault status` and redacted search filters that forward to `vault-agent`; `vault-ops` update/repair/full-clearance workflows remain operator-only. The repo-local scaffold now also has a small plugin config contract for `repoRoot`, `vaultAgentPath`, and `timeoutSeconds`, plus a package-local README and `plugin-config.example.json` under [plugins/llm-vault-openclaw](plugins/llm-vault-openclaw). See [OpenClaw Plugin Scaffold](docs/openclaw-plugin.md).
